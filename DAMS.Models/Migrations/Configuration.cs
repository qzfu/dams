        ...
        protected override void Seed(DAMS.Models.EFDbContext context)
        {
            var users = new List<Users>
            {
                new Users{Id=1,UserId="admin", DeptId=0, Enabled=true, Gender = 0, 
                    UserName="管理员",Password="c4ca4238a0b923820dcc509a6f75849b",
            ...
            var categories = new List<Catagorys> { 
                new Catagorys{Type = 5,ItemValue = "1",ItemText=".swf,.wav,.wma,.flv,.f4v,.rm,.wmv,.avi,.3gp,.mov,.mpg,.mpeg,.rmvb,.mp4",CreatedTime = DateTime.Now},
                new Catagorys{Type = 5,ItemValue = "2",ItemText=".mp3,.m4a",CreatedTime = DateTime.Now},
                new Catagorys{Type = 5,ItemValue = "3",ItemText=".pdf,.rtf,.doc,.docx,.ppt,.pptx,.xls,.xlsx,.txt",CreatedTime = DateTime.Now}
            };
            DbSet<Catagorys> catagorysSet = context.Set<Catagorys>();
            catagorysSet.AddOrUpdate(m => new { m.Type, m.ItemText }, categories.ToArray());
        }
    }
}
